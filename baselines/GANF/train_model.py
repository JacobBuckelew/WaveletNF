...
parser.add_argument('--name', type=str, default= "GANF")
# restore
parser.add_argument('--graph', type=str, default='None')
parser.add_argument('--model', type=str, default='None')
parser.add_argument('--seed', type=int, default=7, help='Random seed to use.')
# model parameters
parser.add_argument('--n_blocks', type=int, default=8, help='Number of blocks to stack in a model (MADE in MAF; Coupling+BN in RealNVP).')
parser.add_argument('--n_components', type=int, default=1, help='Number of Gaussian clusters for mixture of gaussians models.')
parser.add_argument('--hidden_size', type=int, default=16, help='Hidden layer size for MADE (and each MADE block in an MAF).')
...
parser.add_argument('--log_interval', type=int, default=20, help='How often to show loss statistics and save samples.')
...
if args.dataset == "swat":
    ...
    train_loader, val_loader, test_loader, n_sensor = load_wadi(args.batch_size,window_size=60, 
                                                      stride_size=10, train_split=0.60)
elif args.dataset.startswith('machine'):
    train_loader, val_loader, test_loader, n_sensor = load_smd(args.dataset, args.batch_size, window_size=60,
                                                      stride_size=10, train_split=0.60)
elif args.dataset == "psm":
    train_loader, val_loader, test_loader, n_sensor = load_psm(args.batch_size, window_size=60,
                                                      stride_size=10, train_split=0.60)
elif args.dataset == "inrix":
    train_loader, val_loader, test_loader, n_sensor = load_inrix(args.batch_size, window_size=60, stride_size=10, train_split=0.80)
elif args.dataset == "rtds":
    train_loader, val_loader, test_loaders, n_sensor = load_rtds(window_size=60,
                                                       stride_size=10, batch_size=args.batch_size)
