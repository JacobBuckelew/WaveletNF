...
def loader_SWat(batch_size, window_size, stride_size,train_split,label=False):
    print(os.getcwd())
    ...
    val_df = norm_feature.iloc[int(0.6*len(data)):int(0.80*len(data))]
    val_label = labels[int(0.6*len(data)):int(0.80*len(data))]
...
def loader_SWat_OCC(root, batch_size, window_size, stride_size,train_split,label=False):
    print(os.getcwd())
    data = pd.read_csv("SWaT_Dataset_Normal_v1.csv",sep = ',', low_memory=False)
    Timestamp = pd.to_datetime(data["Timestamp"])
    data["Timestamp"] = Timestamp
    data = data.set_index("Timestamp")
    labels = [ int(l!= 'Normal' ) for l in data["Normal/Attack"].values]
